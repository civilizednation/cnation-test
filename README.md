...
최신 정보를 검색해서 답하고 출처 링크를 보여줍니다.

- **자동 검색**: 체크하지 않아도 "오늘/현재/최신/뉴스/날씨/환율/주가" 같은 표현이 있으면 자동으로 검색합니다 (Tavily 키가 있을 때, 첨부파일이 없을 때). 날짜·시각만 묻는 질문은 검색하지 않습니다.
- **🔍 항상 웹 검색**: 체크하면 모든 질문을 검색합니다. 체크 상태는 브라우저에 기억됩니다.
