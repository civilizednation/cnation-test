...
import { streamAI, validateRequest } from './chat.js';
...
export async function handleChat(body, res) {
  ...
  try {
    const request = { prompt: body.prompt, files: body.files ?? [], search: body.search === true };
    for await (const event of streamAI(request)) {
