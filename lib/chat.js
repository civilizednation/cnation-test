...
import { hasWebSearch, webSearch, withSearchResults } from './search.js';
...
export async function* streamAI({ prompt, files = [], search = false }) {
  ...
  // 키가 없거나 실패하면 Gemini 의 Google 검색 연동을 시도합니다.
  let searchSources = [];
  ...
  for (const provider of availableProviders()) {
    ...
    yield { type: 'meta', provider: provider.label, model: started.model, search: searched };
